...
class TensorBase(torch.nn.Module):
    ...
    @torch.no_grad()
    def filtering_rays(self, all_rays, N_samples=256, chunk=10240 * 5, bbox_only=False, return_mask_only=False):
        print('========> filtering rays for multilight ...')
        tt = time.time()
        ...
        for idx_chunk in idx_chunks:
            rays_chunk = all_rays[idx_chunk].to(self.device, dtype=torch.float32)
