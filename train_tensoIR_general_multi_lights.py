...
def reconstruction(args):
    ...
    if args.ckpt is not None:
        ...
        tensoIR = eval(args.model_name)(aabb, 
                                        ...
                                        light_rotation=args.light_rotation,
                                        light_name_list=train_dataset.light_name_list,
    ...
    print(
        "Train tensor sizes:",
        f"rays={tuple(all_rays.shape)} ({tensor_gb(all_rays):.2f} GiB),",
        f"rgbs={tuple(all_rgbs.shape)} ({tensor_gb(all_rgbs):.2f} GiB),",
        f"light_idx={tuple(all_light_idx.shape)} ({tensor_gb(all_light_idx):.2f} GiB)"
    )
    ...
    for iteration in pbar: 
        ...
        flat_ray_idx = valid_ray_indices[rays_idx].long()
        rays_train = all_rays[flat_ray_idx]
        rgb_train = all_rgbs[flat_ray_idx].to(device)
        light_idx_train = all_light_idx[flat_ray_idx].to(device)
